...
with DAG(
    ...
    start_date=pendulum.datetime(2024,12,27, tz='Asia/Seoul'),
    schedule='0 8 * * *',
    catchup=False
) as dag:
    ...
    def get_daily_monitoring_rslt_task():
        ...
        with closing(postgres_hook.get_conn()) as conn: # session
            with closing(conn.cursor()) as cursor: # session 내에서 쿼리를 수행하고 결과를 받아올 수 있는 역할
                with open('/opt/airflow/files/sqls/daily_dag_monitoring.sql', 'r') as sql_file:
                    cursor.execute("SET TIME ZONE 'Asia/Seoul';")
                    sql = '\n'.join(sql_file.readlines()) # 줄당 리스트로 변환 + 리스트 합칠 때 '\n'
                    cursor.execute(sql)
                    ...
                    # 4) 성공 대상
                    done_success_cnt = rslt.query("(failed_cnt == 0) and (run_cnt > 0) and (running_cnt == 0)").shape[0] # 행반환
                    yesterday = pendulum.yesterday('Asia/Seoul').strftime('%Y-%m-%d') # 날짜/시간 > 문자열 (String Format Time)
