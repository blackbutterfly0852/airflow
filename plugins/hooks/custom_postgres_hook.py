...
class CustomPostGresHook(BaseHook):
    ...
    def get_conn(self):
        airflow_conn = BaseHook.get_connection(self.postgres_conn_id) # airflow UI connections
    ...
    def bulk_load(self, table_name, file_name, delimiter: str, is_header: bool, is_replace: bool):
        ...
        # pandas는 숫자형 컬럼이면 숫자형 데이터로 읽어드리고, 날짜형 컬럼이면 날짜로 읽어드림
        ...
        file_df.to_sql(name = table_name,
                       ...
                       schema = 'public',
                       if_exists=if_exists, 
                       index = False  ) # df가 가진 index load x